...
const buttonVariants = cva(
  'inline-flex items-center justify-center gap-2 font-semibold text-sm transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed cursor-pointer select-none',
  {
    variants: {
      variant: {
        default: 'bg-[#7C3AED] text-white hover:bg-[#6D28D9] active:scale-95 shadow-sm',
        outline: 'border border-[var(--border)] text-[var(--text)] bg-[var(--bg-card)] hover:border-[#7C3AED] hover:text-[#7C3AED] active:scale-95',
        ghost: 'text-[var(--text-muted)] hover:text-[var(--text)] hover:bg-[var(--bg-elevated)] active:scale-95',
        danger: 'bg-red-500 text-white hover:bg-red-600 active:scale-95',
        secondary: 'bg-[var(--bg-elevated)] text-[var(--text)] border border-[var(--border)] hover:border-[#7C3AED] hover:text-[#7C3AED] active:scale-95',
      },
      size: {
