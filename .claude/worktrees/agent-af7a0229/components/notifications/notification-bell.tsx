...
export function NotificationBell() {
  ...
  function getIcon(type: string) {
    switch (type) {
      case 'match': return <Users className="w-4 h-4 text-[#7C3AED]" />;
  ...
  return (
    <div className="relative" ref={ref}>
      ...
      >
        ...
        {unreadCount > 0 && (
          <span className="absolute -top-0.5 -right-0.5 w-4 h-4 bg-[#7C3AED] text-white text-[10px] font-bold flex items-center justify-center rounded-full">
          ...
          <div className="flex items-center justify-between p-3 border-b border-[var(--border)]">
            ...
            <div className="flex items-center gap-2">
              {unreadCount > 0 && (
                <button
                  onClick={markAllRead}
                  className="text-[10px] text-[#888888] hover:text-[#7C3AED] flex items-center gap-1 uppercase tracking-wider"
                >
              ...
              notifications.map((n) => (
                <div
                  ...
                  className={`flex items-start gap-3 p-3 border-b border-[var(--border)] transition-colors hover:bg-[#161616] ${
                    !n.read ? 'bg-[rgba(124,58,237,0.05)]' : ''
                  }`}
                >
