    ...
    <div className="flex flex-col gap-2">
      <form onSubmit={handleSearch} className="flex gap-2">
        ...
        <button
          type="submit"
          disabled={searching}
          className="px-4 py-2.5 bg-[#7C3AED] text-white text-xs font-semibold uppercase tracking-wider hover:shadow-[0_0_15px_rgba(124,58,237,0.4)] disabled:opacity-50"
        >
        ...
        {!ready && (
          <div className="absolute inset-0 bg-[var(--bg)] flex items-center justify-center border border-[var(--border)]">
            <div className="flex items-center gap-2 text-[#888888] text-sm">
              <MapPin className="w-4 h-4 animate-bounce text-[#7C3AED]" />
              Loading map...
            </div>
          </div>
        )}
