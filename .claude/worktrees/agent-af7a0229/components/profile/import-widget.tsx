          ...
          <div>
            ...
            {showCookieHelp && (
              <div className="mb-3 p-3 bg-[var(--bg)] border border-[var(--border)] text-xs text-[#888888] space-y-1">
                ...
                <p>3. Skopiuj wartości: <code className="text-[#7C3AED]">GARMIN-SSO-GUID</code>, <code className="text-[#7C3AED]">JWT_FG</code>, <code className="text-[#7C3AED]">SESSIONID</code></p>
              ...
              <div className="grid grid-cols-2 gap-2 mb-2">
                {result.athleteName && (
                  <div className="col-span-2 text-xs text-[#888888]">
                    Athlete: <span className="text-white">{result.athleteName}</span>
                ...
                {result.stats.ytdRunKm > 0 && (
                  <div className="text-xs text-[#888888]">
                    YTD Running: <span className="text-white">{result.stats.ytdRunKm}km</span>
                  </div>
                )}
                {result.stats.ytdRideKm > 0 && (
                  <div className="text-xs text-[#888888]">
                    YTD Cycling: <span className="text-white">{result.stats.ytdRideKm}km</span>
                  </div>
                )}
                {result.stats.recentRunKm > 0 && (
                  <div className="text-xs text-[#888888]">
                    Recent 4w Run: <span className="text-white">{result.stats.recentRunKm}km</span>
                  </div>
                )}
              </div>
            )}
    ...
    <div className="flex flex-col gap-6">
      ...
      <div className="bg-[var(--bg-card)] border border-[var(--border)] p-5">
        <div className="flex items-center gap-3 mb-4">
          ...
          <div>
            <h3 className="text-white font-medium text-sm">Import from Strava</h3>
            <p className="text-[10px] text-[#555555]">Sync your activities, FTP, and weekly volume</p>
          </div>
          {loadingStatus ? (
            ...
            <div className="ml-auto flex items-center gap-1.5 px-2.5 py-1 border border-[var(--border)] bg-[var(--bg)]">
              ...
              <span className="text-xs text-[#555555]">Not connected</span>
            </div>
          )}
        </div>
        ...
        {stravaStatus?.connected && (
          <div className="mb-4 p-3 bg-[var(--bg)] border border-[var(--border)] flex items-center justify-between">
            <div className="text-xs text-[#888888]">
              <span className="text-white font-medium">{stravaStatus.activityCount}</span> activities synced
        ...
        <form onSubmit={handleStravaImport}>
          ...
          <p className="text-[10px] text-[#555555] mt-1.5">
            Paste your public Strava profile URL to import stats. For full data access,{' '}
        ...
        {!stravaStatus?.connected && !loadingStatus && (
          <div className="mt-4 pt-4 border-t border-[var(--border)]">
            ...
            <p className="text-[10px] text-[#555555] mt-2 text-center">
              Full sync: activities, FTP, weekly volume, auto pace verification
            </p>
          </div>
        )}
