      ...
      {loading ? (
            ...
            <div>
              ...
              <div className="flex items-center gap-1.5 mt-0.5">
                ...
                {myCheckin.workoutType && (
                  <span className="text-xs text-[#7C3AED] border border-[#7C3AED]/30 px-1.5 py-0.5">
                    {myCheckin.workoutType}
                  </span>
                )}
              </div>
                ...
                {others.slice(0, 5).map((o) => (
                  <div key={o.checkin.id} className="flex items-center gap-2">
                    ...
                    {o.checkin.workoutType && (
                      <span className="text-[10px] text-[#7C3AED] border border-[#7C3AED]/20 px-1">
                        {o.checkin.workoutType}
                      </span>
                    )}
                  </div>
          ...
          <div className="grid grid-cols-2 gap-1.5 mb-3">
            {WORKOUT_TYPES.map((type) => (
              <button
                ...
                disabled={checking}
                className="py-1.5 text-xs font-semibold uppercase tracking-wider border border-[var(--border)] text-[#888888] hover:text-white hover:border-[#7C3AED] transition-all disabled:opacity-50"
              >
                {type}
              </button>
            ))}
        ...
        <div>
          ...
          <button
            ...
            disabled={checking}
            className="w-full py-2.5 bg-[#7C3AED] text-white text-xs font-semibold uppercase tracking-wider hover:shadow-[0_0_15px_rgba(124,58,237,0.4)] disabled:opacity-50 transition-all flex items-center justify-center gap-2"
          >
