  ...
  return (
    <div>
      ...
      {sets.map((set, i) => (
        <div key={i} className="grid grid-cols-[40px_1fr_1fr_40px] gap-2 mb-2">
          ...
          <input
            ...
            className="bg-[var(--bg)] border border-[var(--border)] text-[var(--text)] px-2 py-2 text-sm text-center focus:border-[#7C3AED] focus:outline-none"
            placeholder="0"
          />
          <input
            type="number"
      ...
      <button
        type="button"
        onClick={addSet}
        className="flex items-center gap-2 text-xs text-[#7C3AED] hover:text-[#8B5CF6] mt-2 transition-colors"
      >
      ...
      {showOneRM && maxSet.rm > 0 && maxSet.reps > 1 && (
        <div className="mt-3 p-2 bg-[rgba(124,58,237,0.08)] border border-[rgba(124,58,237,0.2)]">
          <p className="text-xs text-[#888888]">
            Estimated 1RM:{' '}
            <span className="text-[#7C3AED] font-bold">{maxSet.rm}kg</span>
