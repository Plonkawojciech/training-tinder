...
export function PRCard({ exerciseName, weightKg, reps, achievedAt, notes, isBig4 }: PRCardProps) {
  ...
  const date = new Date(achievedAt).toLocaleDateString('en-US', {
    month: 'short',
    day: 'numeric',
    year: 'numeric',
  });
  ...
  return (
    <div
      className="bg-[var(--bg-card)] border p-4 transition-all hover:border-[#7C3AED]"
      style={{ borderColor: isBig4 ? 'rgba(124,58,237,0.4)' : 'var(--border)' }}
    >
      <div className="flex items-start justify-between mb-2">
        <div className="flex items-center gap-2">
          <Trophy
            className="w-4 h-4 shrink-0"
            style={{ color: isBig4 ? '#FFD700' : '#7C3AED' }}
          />
        ...
        {reps > 1 && (
          <div className="mb-0.5">
            <div className="flex items-center gap-1">
              ...
              <p className="text-sm text-[#7C3AED] font-bold">~{oneRM}kg</p>
            </div>
