...
export function SwipeCard({
  ...
  const matchColor = score >= 80 ? '#00E676' : score >= 60 ? '#FFD700' : score >= 40 ? '#FF9800' : '#7C3AED';
  const primarySport = athlete.sportTypes[0];
  const sportColor = primarySport ? getSportBadgeColor(primarySport) : '#7C3AED';
    ...
    >
      ...
      >
        ...
        <div className="absolute bottom-0 left-0 right-0" style={{ padding: '0 20px 18px' }}>
          ...
          {/* Stats row */}
          {(athlete.pacePerKm || athlete.weeklyKm) && (
            <div style={{
              display: 'flex',
              gap: 16,
              marginBottom: 16,
