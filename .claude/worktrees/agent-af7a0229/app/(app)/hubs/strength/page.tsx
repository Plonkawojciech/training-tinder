  ...
  return (
    <div className="p-4 md:p-6 max-w-5xl mx-auto pb-24 md:pb-6">
      ...
      <div className="flex items-center gap-3 mb-8">
        ...
        <div>
          <h1 className="font-display text-3xl text-white tracking-wider">STRENGTH HUB</h1>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6">
        {[
          { label: 'Workouts Logged', value: String(workouts.length), icon: Dumbbell, color: '#7C3AED' },
          { label: 'PRs Tracked', value: String(prs.length), icon: Trophy, color: '#FFD700' },
          { label: 'Current Split', value: splits.length > 0 ? SPLIT_LABELS[splits[0]]?.split('/')[0] ?? splits[0] : 'Not Set', icon: Calendar, color: '#00CC88' },
          { label: 'Gym', value: profile?.gymName ?? 'Not Set', icon: MapPin, color: '#888888' },
        ].map((stat) => {
      ...
      {splits.length > 0 && (
        <div className="bg-[var(--bg-card)] border border-[var(--border)] p-5 mb-6">
          <h2 className="font-display text-sm text-[#888888] tracking-wider mb-4">TRAINING SPLITS</h2>
          <div className="flex flex-wrap gap-2">
            {splits.map((split) => {
              const color = SPLIT_COLORS[split] ?? '#7C3AED';
              return (
                <span
                  key={split}
      ...
      <div className="bg-[var(--bg-card)] border border-[var(--border)] p-5 mb-6">
        <div className="flex items-center justify-between mb-4">
          ...
          <Link href="/gym" className="text-xs text-[#7C3AED] hover:text-[#8B5CF6] flex items-center gap-1 transition-colors">
      ...
      {prs.length > 0 && (
        <div className="bg-[var(--bg-card)] border border-[var(--border)] p-5 mb-6">
          <h2 className="font-display text-sm text-[#888888] tracking-wider mb-4">PERSONAL RECORDS</h2>
          <div className="flex flex-col gap-2">
            {prs.map((pr, i) => (
              <div key={pr.exerciseName} className="flex items-center gap-4 p-3 bg-[var(--bg-card)] border border-[var(--border)]">
                <span className="font-display text-lg text-[#7C3AED] w-6 shrink-0">#{i + 1}</span>
