...
const SPORT_COLORS = ['#7C3AED', '#00CC88', '#FFD700', '#A78BFA', '#CC0044', '#0088FF'];
  ...
  return (
    <div className="p-4 md:p-6 max-w-5xl mx-auto pb-24 md:pb-6">
      ...
      <div className="flex items-center gap-3 mb-8">
        ...
        <div>
          <h1 className="font-display text-3xl text-white tracking-wider">ANALYTICS HUB</h1>
          <p className="text-[#888888] text-sm">Progress · Trends · Records</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-3 gap-3 mb-6">
        {[
          { label: 'Total km', value: totalKm.toFixed(0), icon: Activity, color: '#7C3AED' },
          { label: 'Workouts', value: String(totalWorkouts), icon: Dumbbell, color: '#00CC88' },
          { label: 'PRs', value: String(totalPRs), icon: Trophy, color: '#FFD700' },
        ].map((s) => {
          const Icon = s.icon;
          return (
      ...
      <div className="bg-[var(--bg-card)] border border-[var(--border)] p-5 mb-6">
        <h2 className="font-display text-sm text-[#888888] tracking-wider mb-4">WEEKLY DISTANCE (km)</h2>
        {loading ? (
          <div className="h-40 skeleton" />
        ) : !stravaConnected ? (
          <div className="text-center py-8">
            <Activity className="w-10 h-10 text-[#2A2A2A] mx-auto mb-2" />
            <p className="text-[#888888] text-sm mb-2">Connect Strava for activity data</p>
            <Link href="/profile" className="text-xs text-[#7C3AED] hover:underline">Connect Strava →</Link>
          </div>
        ) : weeklyData.length === 0 ? (
          <p className="text-[#888888] text-sm text-center py-6">No activity data yet</p>
        ) : (
            ...
            {weeklyData.map((w) => {
              ...
              return (
                <div key={w.week} className="flex-1 flex flex-col items-center gap-1">
                  ...
                  <div
                    className="w-full bg-[#7C3AED] transition-all duration-500 hover:bg-[#8B5CF6]"
      ...
      {sportBreakdown.length > 0 && (
        <div className="bg-[var(--bg-card)] border border-[var(--border)] p-5 mb-6">
          <h2 className="font-display text-sm text-[#888888] tracking-wider mb-4">SPORT BREAKDOWN</h2>
      ...
      <div className="bg-[var(--bg-card)] border border-[var(--border)] p-5 mb-6">
        <div className="flex items-center justify-between mb-4">
          <h2 className="font-display text-sm text-[#888888] tracking-wider">PERSONAL RECORDS</h2>
          <Link href="/stats" className="text-xs text-[#7C3AED] hover:text-[#8B5CF6] flex items-center gap-1 transition-colors">
        ...
        {loading ? (
          ...
          <p className="text-[#888888] text-sm text-center py-4">No PRs tracked yet. Log workouts to set records!</p>
        ) : (
          <div className="flex flex-col gap-2">
            {prs.slice(0, 8).map((pr, i) => (
              <div key={pr.exerciseName} className="flex items-center gap-4 p-3 bg-[var(--bg-card)] border border-[var(--border)]">
                <span className="font-display text-base text-[#7C3AED] w-6 shrink-0">#{i + 1}</span>
      ...
      <div className="bg-[var(--bg-card)] border border-[var(--border)] p-5">
        <h2 className="font-display text-sm text-[#888888] tracking-wider mb-4">WORKOUT FREQUENCY (last 12 weeks)</h2>
        {loading ? (
          <div className="h-24 skeleton" />
        ) : workouts.length === 0 ? (
          <p className="text-[#888888] text-sm text-center py-4">No workout data yet</p>
        ) : (() => {
              ...
              {weeks.map(([week, count]) => {
                ...
                return (
                  <div key={week} className="flex-1 flex flex-col items-center gap-1">
                    <div
                      className="w-full bg-[rgba(124,58,237,0.6)] hover:bg-[#7C3AED] transition-colors"
