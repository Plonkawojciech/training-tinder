  ...
  return (
    <div className="p-4 md:p-6 max-w-5xl mx-auto pb-24 md:pb-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6">
        {[
          { label: 'This Week', value: weeklyKm.toFixed(1) + ' km', icon: Route, color: '#7C3AED' },
          { label: 'Activities', value: String(activities.length), icon: Zap, color: '#00CC88' },
          { label: 'Move Time', value: formatDuration(activities.reduce((s, a) => s + a.moving_time, 0)), icon: Clock, color: '#FFD700' },
          { label: 'Weekly Goal', value: weeklyGoal + ' km', icon: TrendingUp, color: '#7C3AED' },
        ].map((stat) => {
      ...
      <div className="bg-[var(--bg-card)] border border-[var(--border)] p-5 mb-6">
        <div className="flex items-center justify-between mb-3">
          <h2 className="font-display text-sm text-[#888888] tracking-wider">WEEKLY DISTANCE GOAL</h2>
        ...
        <p className="text-xs text-[#555555] mt-2">{Math.round(progressPct)}% complete</p>
      </div>

      {/* Strava section */}
      <div className="bg-[var(--bg-card)] border border-[var(--border)] p-5 mb-6">
        <div className="flex items-center justify-between mb-4">
          <h2 className="font-display text-sm text-[#888888] tracking-wider">RECENT ACTIVITIES</h2>
        ...
        {loading ? (
          ...
          <div className="text-center py-8">
            ...
            <p className="text-[#888888] text-sm mb-3">Connect Strava to see your activities</p>
            <Link href="/profile">
              <Button size="sm">Connect Strava</Button>
            </Link>
          </div>
        ) : activities.length === 0 ? (
          <p className="text-[#888888] text-sm text-center py-6">No recent activities from Strava</p>
        ) : (
          <div className="flex flex-col gap-2">
            {activities.map((activity) => (
              ...
              >
                <div className="w-8 h-8 bg-[rgba(124,58,237,0.1)] flex items-center justify-center shrink-0">
