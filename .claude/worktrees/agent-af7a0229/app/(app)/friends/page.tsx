...
export default function FriendsPage() {
  ...
  async function handleSearch(e: React.FormEvent) {
    ...
    try {
      ...
      else setSearchError(data.error ?? 'Blad');
    ...
    <div style={{ maxWidth: 600, margin: '0 auto', padding: '24px 16px 80px' }}>
      ...
      <div style={card}>
        <div style={{ fontWeight: 700, color: 'var(--text)', marginBottom: 14, display: 'flex', alignItems: 'center', gap: 8 }}>
          <UserPlus style={{ width: 18, height: 18, color: '#7C3AED' }} />
        ...
        <form onSubmit={handleSearch} style={{ display: 'flex', gap: 10 }}>
          ...
          <button type="submit" disabled={searching || !searchQ.trim()} style={{
            ...
            background: searchQ.trim() ? '#7C3AED' : 'rgba(255,255,255,0.08)',
      ...
      {pendingReceived.length > 0 && (
        <div style={card}>
          <div style={{ fontWeight: 700, color: 'var(--text)', marginBottom: 12, display: 'flex', alignItems: 'center', gap: 8 }}>
            <span style={{ background: '#7C3AED', borderRadius: '50%', width: 20, height: 20, display: 'inline-flex', alignItems: 'center', justifyContent: 'center', fontSize: 11, fontWeight: 700, color: 'white' }}>{pendingReceived.length}</span>
          ...
          {pendingReceived.map(r => (
            <div key={r.id} style={rowStyle}>
              ...
              <div style={{ flex: 1 }}>
                <div style={{ color: 'var(--text)', fontWeight: 600, fontSize: 14 }}>{r.otherUser?.username ?? 'Athlete'}</div>
