  ...
  return (
    <div className="p-4 md:p-6 max-w-3xl mx-auto">
      ...
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          <h1 className="font-display text-3xl text-white tracking-wider">IMPORT DATA</h1>
          <p className="text-[#888888] text-sm">Connect your training accounts to auto-fill your profile</p>
        </div>
      </div>
      ...
      {!loading && userProfile && (
        <div className="bg-[var(--bg-card)] border border-[var(--border)] p-5 mb-6">
          <div className="flex items-center justify-between mb-4">
            <div>
              <h2 className="font-display text-sm text-[#888888] tracking-wider">PROFILE COMPLETENESS</h2>
              <p className="text-xs text-[#555555] mt-0.5">{completionCount}/{totalCount} key fields filled</p>
            </div>
            <div className="flex items-center gap-3">
              ...
              <div className="w-20 h-2 bg-[var(--bg-elevated)] rounded-full overflow-hidden">
                <div
                  ...
                  style={{
                    width: `${completionPct}%`,
                    background: completionPct >= 80 ? '#22C55E' : completionPct >= 50 ? '#EAB308' : '#7C3AED',
                  }}
                />
              </div>
            </div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
            {[
              ...
              { key: 'pace', label: 'Threshold Pace', value: userProfile.pacePerKm ? formatPace(userProfile.pacePerKm) : null },
              { key: 'weeklyKm', label: 'Weekly KM', value: userProfile.weeklyKm ? `${userProfile.weeklyKm}km` : null },
              { key: 'city', label: 'City', value: userProfile.city },
            ].map(({ key, label, value }) => (
              <div
                key={key}
          ...
          {completionPct < 100 && (
            <div className="mt-4 flex items-center justify-between">
              <p className="text-xs text-[#555555]">
                Fill in missing data manually in Training Zones or via import below
              </p>
              <Link href="/training">
      ...
      <div className="bg-[var(--bg-card)] border border-[var(--border)] p-5 mb-6">
        <h2 className="font-display text-sm text-[#888888] tracking-wider mb-4">CONNECTED ACCOUNTS</h2>
        ...
        {loading ? (
          ...
          <div>
            <ConnectionStatus
              ...
              detail={stravaStatus?.connected ? `${stravaStatus.activityCount} activities` : undefined}
            />
            <ConnectionStatus
      ...
      <div className="bg-[var(--bg)] border border-[var(--border)] p-4 mb-6 flex items-start gap-3">
        ...
        <div>
          <p className="text-xs text-[#888888] mb-1">
            <span className="text-white">No integration?</span> No problem. You can manually enter your FTP, max heart rate, and pace in the{' '}
            <Link href="/training" className="text-[#7C3AED] hover:underline">
              Training Zones
            </Link>{' '}
            page, or fill in your full profile on the{' '}
            <Link href="/profile" className="text-[#7C3AED] hover:underline">
              Profile
            </Link>{' '}
            page.
          </p>
          <p className="text-[10px] text-[#444444]">
            All performance data is optional and only used to improve match quality and zone calculations.
          </p>
        </div>
      </div>
