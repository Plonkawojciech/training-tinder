...
function SeriesCard({
  ...
  async function handleDelete() {
    if (!confirm('Delete this recurring series? This cannot be undone.')) return;
    setDeleting(true);
    try {
  ...
  return (
    ...
    >
      ...
      <div className="bg-[var(--bg-card)] border border-[var(--border)] px-3 py-2 flex items-center gap-2">
        ...
        <span className="text-xs text-[#888888]">Next:</span>
        <span className="text-xs text-white font-medium">
          {nextOcc.toLocaleDateString('en-US', {
            weekday: 'short',
            month: 'short',
            day: 'numeric',
          })} at {series.time}
      ...
      {isOwn && (
        <div className="flex items-center gap-1.5">
          ...
          <span className="text-xs text-[#888888]">Your series</span>
        </div>
      )}
    </div>
  );
    ...
    <div className="p-4 md:p-6 max-w-4xl mx-auto">
      ...
      <div className="flex items-center justify-between gap-4 mb-6">
        <div className="flex items-center gap-3">
          <Repeat className="w-6 h-6 text-[#7C3AED]" />
          <div>
            <h1 className="font-display text-3xl text-white tracking-wider">RECURRING SERIES</h1>
            <p className="text-[#888888] text-sm">Regular group sessions you can join</p>
          </div>
        </div>
        <Button
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center justify-center py-20 gap-4 border border-dashed border-[var(--border)]">
          ...
          <h3 className="font-display text-xl text-[#888888]">NO RECURRING SERIES</h3>
          <p className="text-[#888888] text-sm text-center max-w-sm">
            You haven&apos;t created any recurring series yet. Start one to bring athletes together week after week.
          </p>
