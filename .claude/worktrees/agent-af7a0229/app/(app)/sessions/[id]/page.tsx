...
import { GroupChat } from '@/components/sessions/group-chat';
      ...
      <div style={{ background: 'var(--bg-card)', borderRadius: 24, padding: '20px', marginBottom: 16, boxShadow: 'var(--shadow-card)' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginBottom: 16 }}>
          {[
            ...
            <div key={label} style={{ display: 'flex', alignItems: 'center', gap: 8, background: 'var(--bg-elevated)', borderRadius: 12, padding: '10px 12px' }}>
              <Icon style={{ width: 15, height: 15, color: '#7C3AED', flexShrink: 0 }} />
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, background: 'var(--bg-elevated)', borderRadius: 12, padding: '10px 12px', gridColumn: '1 / -1' }}>
            <MapPin style={{ width: 15, height: 15, color: '#7C3AED', flexShrink: 0 }} />
        ...
        <div style={{ marginBottom: 16 }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 8 }}>
            ...
            <span style={{ fontSize: 13, fontWeight: 700, color: isFull ? 'var(--text-muted)' : '#7C3AED' }}>
                  ...
                  <div className="flex flex-col gap-1.5">
                    {session.stops.map((stop, i) => (
                      <div key={i} className="flex items-center gap-2 text-sm">
                        <span className="w-5 h-5 rounded-full bg-[#7C3AED] text-white text-xs flex items-center justify-center font-bold shrink-0">{i + 1}</span>
        ...
        {session.gpxUrl && (
          <a
            ...
            style={{ display: 'inline-flex', alignItems: 'center', gap: 6, fontSize: 13, color: '#7C3AED', marginBottom: 16 }}
          >
        ...
        {tabs.map((tab) => (
          <button
            ...
            style={{
              ...
              background: activeTab === tab.id ? '#7C3AED' : 'var(--bg-card)',
              color: activeTab === tab.id ? 'white' : 'var(--text-muted)',
              boxShadow: activeTab === tab.id ? '0 4px 12px rgba(124,58,237,0.3)' : 'var(--shadow-card)',
          ...
          <div style={{ background: 'var(--bg-card)', borderRadius: 20, padding: '16px', marginBottom: 12, boxShadow: 'var(--shadow-card)' }}>
            ...
            <div className="flex items-center gap-3">
              ...
              <Link
                ...
                className="font-semibold text-white hover:text-[#7C3AED] transition-colors text-sm"
              >
                        ...
                        <div key={p.userId} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 12px', background: 'rgba(251,191,36,0.06)', borderRadius: 12 }}>
                          ...
                          <div style={{ display: 'flex', gap: 6 }}>
                            <button onClick={() => handleMemberAction(p.userId, 'accept')} disabled={memberActionLoading === p.userId + 'accept'}
                              style={{ padding: '6px 14px', borderRadius: 99, border: 'none', cursor: 'pointer', fontSize: 12, fontWeight: 700, background: '#7C3AED', color: 'white', opacity: memberActionLoading === p.userId + 'accept' ? 0.5 : 1 }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                  {session.participants.filter((p) => p.status === 'accepted' || p.status === 'host').map((p) => (
                    <Link key={p.userId} href={`/profile/${p.userId}`} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '8px 10px', borderRadius: 12, textDecoration: 'none', transition: 'background 0.15s' }}>
                      ...
                      {p.status === 'host' && <span style={{ fontSize: 10, fontWeight: 700, color: '#7C3AED', background: 'rgba(124,58,237,0.12)', padding: '3px 8px', borderRadius: 99 }}>HOST</span>}
                    </Link>
                  ))}
          ...
          <div className="flex gap-0.5">
            {[1,2,3,4,5].map((s) => (
              <Star
                key={s}
                className="w-4 h-4"
                fill={s <= Math.round(avg) ? '#7C3AED' : 'transparent'}
                stroke={s <= Math.round(avg) ? '#7C3AED' : '#555555'}
              />
            ))}
          </div>
            ...
            <div className="flex items-center gap-1 mb-1.5">
              {[1,2,3,4,5].map((s) => (
                <Star
                  ...
                  fill={s <= review.rating ? '#7C3AED' : 'transparent'}
                  stroke={s <= review.rating ? '#7C3AED' : '#555555'}
                />
              ))}
            </div>
            {review.comment && (
