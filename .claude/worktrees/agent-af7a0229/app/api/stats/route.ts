...
export async function GET(request: Request) {
  ...
  try {
    ...
    if (type === 'summary') {
      ...
      // Total sets this month — single query instead of N+1
      const monthWorkoutIds = monthWorkouts.map((w) => w.id);
      const monthExercises = monthWorkoutIds.length > 0
        ? await db
            .select({ sets: exercises.sets })
            .from(exercises)
            .where(inArray(exercises.workoutLogId, monthWorkoutIds))
        : [];
      const totalSets = monthExercises.reduce((sum, e) => sum + e.sets, 0);
